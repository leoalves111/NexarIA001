-- REORGANIZACAO COMPLETA DO BANCO DE DADOS - GRUPO 2 (VERSAO FINAL)
...
DO $$
BEGIN
    RAISE NOTICE '=== INICIANDO REORGANIZACAO DO BANCO DE DADOS - GRUPO 2 (VERSAO FINAL) ===';
...
-- 2. REORGANIZAR TABELA SAVED_CONTRACTS (REMOVER DUPLICACOES) - VERSAO FINAL
DO $$
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables t WHERE t.table_name = 'saved_contracts') THEN
        ...
        -- PRIMEIRO: REMOVER TODAS AS POLITICAS ANTIGAS QUE DEPENDEM DAS COLUNAS
        ...
        -- SEGUNDO: MIGRAR DADOS DAS COLUNAS DUPLICADAS
        ...
        -- TERCEIRO: MIGRAR userid PARA user_id COM CONVERSAO DE TIPO SEGURA
        ...
        -- QUARTO: REMOVER COLUNAS DUPLICADAS (agora sem dependencias)
        ...
        -- QUINTO: CONFIGURAR CAMPOS OBRIGATORIOS
        ...
        -- SEXTO: ADICIONAR FOREIGN KEY PARA PROFILES
        ...
        -- SETIMO: HABILITAR RLS E CRIAR NOVAS POLITICAS
...
VALUES (
    'database-reorganization-group2-complete', 
    'Reorganização completa do Grupo 2 - versão final sem dependências',
    NOW()
...
BEGIN
    RAISE NOTICE '=== RELATORIO FINAL - REORGANIZACAO GRUPO 2 (VERSAO FINAL) ===';
    ...
    RAISE NOTICE '';
    RAISE NOTICE 'RESUMO GRUPO 2 (VERSAO FINAL):';
    ...
    RAISE NOTICE '=== FIM DO RELATORIO GRUPO 2 (VERSAO FINAL) ===';
END $$;
