-- REORGANIZACAO COMPLETA DO BANCO DE DADOS - GRUPO 2
...
DO $$
BEGIN
    RAISE NOTICE '=== INICIANDO REORGANIZACAO DO BANCO DE DADOS - GRUPO 2 ===';
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables t WHERE t.table_name = 'saved_contracts') THEN
        -- REMOVER COLUNAS DUPLICADAS (manter versao snake_case)
        
        -- Migrar dados das colunas camelCase para snake_case antes de remover
        ...
        -- Remover colunas duplicadas (camelCase)
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables t WHERE t.table_name = 'prompt_profiles') THEN
        ...
        ALTER TABLE prompt_profiles ALTER COLUMN userid SET NOT NULL;
        
        -- Renomear userid para user_id para padronizar
        ALTER TABLE prompt_profiles RENAME COLUMN userid TO user_id;
...
BEGIN
    RAISE NOTICE '=== RELATORIO FINAL - REORGANIZACAO GRUPO 2 ===';
    ...
    RAISE NOTICE '=== FIM DO RELATORIO GRUPO 2 ===';
END $$;
