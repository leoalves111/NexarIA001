-- REORGANIZACAO COMPLETA DO BANCO DE DADOS - GRUPO 2 (VERSAO FINAL CORRIGIDA)
...
DO $$
BEGIN
    RAISE NOTICE '=== INICIANDO REORGANIZACAO DO BANCO DE DADOS - GRUPO 2 (VERSAO FINAL CORRIGIDA) ===';
...
-- 2. REORGANIZAR TABELA SAVED_CONTRACTS (REMOVER DUPLICACOES)
DO $$
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables t WHERE t.table_name = 'saved_contracts') THEN
        ...
        -- MIGRAR DADOS DAS COLUNAS DUPLICADAS
        
        -- Verificar se as colunas duplicadas existem e migrar dados
        ...
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'saved_contracts' AND column_name = 'userid') THEN
            -- Adicionar coluna user_id se nao existir
            IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'saved_contracts' AND column_name = 'user_id') THEN
        ...
        -- REMOVER COLUNAS DUPLICADAS (agora sem dependencias)
        ...
        -- CONFIGURAR CAMPOS OBRIGATORIOS
        ...
        -- HABILITAR RLS E CRIAR NOVAS POLITICAS
        ALTER TABLE saved_contracts ENABLE ROW LEVEL SECURITY;
        
        -- Politicas RLS especificas (usando user_id)
...
-- 3. REORGANIZAR TABELA PROMPT_PROFILES (CORRIGIR DEPENDENCIAS)
DO $$
BEGIN
    RAISE NOTICE '3. Reorganizando tabela PROMPT_PROFILES (corrigindo dependencias)...';
    
    IF EXISTS (SELECT 1 FROM information_schema.tables t WHERE t.table_name = 'prompt_profiles') THEN
        
        -- MIGRAR userid PARA user_id
        ...
        -- REMOVER COLUNA userid (agora sem dependencias)
        ...
        -- Garantir que user_id nao seja nulo
        UPDATE prompt_profiles SET user_id = auth.uid() WHERE user_id IS NULL;
        ...
        -- Politicas RLS especificas (usando user_id)
        CREATE POLICY prompt_profiles_select_policy ON prompt_profiles FOR SELECT USING (auth.uid() = user_id);
        CREATE POLICY prompt_profiles_insert_policy ON prompt_profiles FOR INSERT WITH CHECK (auth.uid() = user_id);
        CREATE POLICY prompt_profiles_update_policy ON prompt_profiles FOR UPDATE USING (auth.uid() = user_id);
        CREATE POLICY prompt_profiles_delete_policy ON prompt_profiles FOR DELETE USING (auth.uid() = user_id);
...
-- 4. REORGANIZAR TABELA SUBSCRIPTIONS
DO $$
BEGIN
    RAISE NOTICE '4. Reorganizando tabela SUBSCRIPTIONS...';
...
-- 5. REORGANIZAR TABELA PAYMENT_HISTORY
DO $$
BEGIN
    RAISE NOTICE '5. Reorganizando tabela PAYMENT_HISTORY...';
...
-- 7. REORGANIZAR TABELA LOGIN_ATTEMPTS
DO $$
BEGIN
    RAISE NOTICE '7. Reorganizando tabela LOGIN_ATTEMPTS...';
...
-- 8. CRIAR TRIGGERS PARA TIMESTAMPS AUTOMATICOS
...
BEGIN
    ...
    LOOP
        IF EXISTS (SELECT 1 FROM information_schema.tables t WHERE t.table_name = current_table_name) THEN
            EXECUTE format('DROP TRIGGER IF EXISTS update_%s_updated_at ON %s', current_table_name, current_table_name);
            EXECUTE format('CREATE TRIGGER update_%s_updated_at 
                           BEFORE UPDATE ON %s 
                           FOR EACH ROW EXECUTE FUNCTION update_updated_at_column()', current_table_name, current_table_name);
            RAISE NOTICE 'Trigger de updated_at criado para %', current_table_name;
        END IF;
    END LOOP;
END $$;

-- 9. CRIAR FUNCOES DE UTILIDADE
...
-- 10. CONCEDER PERMISSOES ADEQUADAS
...
-- 11. REGISTRAR MIGRACAO
INSERT INTO migration_history (name, description, executed_at) 
VALUES (
    'database-reorganization-group2-final-fixed', 
    'Reorganização completa do Grupo 2 - versão final corrigida sem dependências e conflitos de nomes',
    NOW()
) ON CONFLICT (name) DO UPDATE SET 
    executed_at = NOW(),
    description = EXCLUDED.description;

-- 12. RELATORIO FINAL DO GRUPO 2
DO $$
DECLARE
    current_table_name TEXT;
    rls_enabled BOOLEAN;
    ...
    duplicate_columns_removed INTEGER := 5; -- nomePersonalizado, tipoPersonalizado, leisSelecionadas, userid (2x)
BEGIN
    RAISE NOTICE '=== RELATORIO FINAL - REORGANIZACAO GRUPO 2 (VERSAO FINAL CORRIGIDA) ===';
    
    -- Verificar cada tabela do grupo 2
    FOR current_table_name IN VALUES ('profiles'), ('saved_contracts'), ('subscriptions'), 
                             ('payment_history'), ('prompt_profiles'), ('migration_history'), ('login_attempts')
    LOOP
        IF EXISTS (SELECT 1 FROM information_schema.tables t WHERE t.table_name = current_table_name) THEN
    ...
    RAISE NOTICE '';
    RAISE NOTICE 'RESUMO GRUPO 2 (VERSAO FINAL CORRIGIDA):';
    ...
    RAISE NOTICE 'Triggers de timestamp: % criados', total_tables - 2; -- migration_history e login_attempts nao tem updated_at
    ...
    IF total_tables = 7 AND tables_with_rls = 7 THEN
        ...
        RAISE NOTICE '✅ Dependências de políticas resolvidas';
    ...
    RAISE NOTICE '=== FIM DO RELATORIO GRUPO 2 (VERSAO FINAL CORRIGIDA) ===';
END $$;
